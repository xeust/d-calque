    ...
    var require_base = __commonJS((exports, module) => {
        ...
        var Base = class extends BaseService {
            ...
            async put(item, key) {
                ...
                if (key)
                    payload["key"] = key;
                const { status, response } = await this.request(`/${this.tableName}/items`, {
                    items: [payload]
                }, "PUT");
            ...
            async *fetch(query = [], pages = 10, buffer = void 0) {
                ...
                do {
                    ...
                    const { status, response } = await this.request(`/${this.tableName}/query`, payload, "POST");
            ...
            async update(updates, key) {
                ...
                for (let [key2, value] of Object.entries(updates)) {
                ...
                key = encodeURIComponent(key);
                const { status, response } = await this.request(`/${this.tableName}/items/${key}`, payload, "PATCH");
                if (status == 200) {
                    return null;
                } else if (status == 404) {
